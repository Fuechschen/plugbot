...
let utils = {
    replace: (string, replacer) => {
        for (let key in replacer)if (replacer.hasOwnProperty(key)) string = string.replace(new RegExp(`&{${key}}`, 'g'), replacer[key] || '');
        return string;
    },
    ...
    rolename: (role) => {
        ...
        return langfile.setstaff.roles.undef;
    },
    blacklistReason: (string) => {
        for (let key in langfile.blacklist.reasons)if (langfile.blacklist.reasons.hasOwnProperty(key)) string = string.replace(new RegExp(`#${key}`, 'g'), langfile.blacklist.reasons[key]);
        return string;
    },
