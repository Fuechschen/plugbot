        ...
        afk: new CronJob('0 */10 * * * *', () => {
            ...
            if (config.afk.enabled) {
                ...
                promise = db.models.User.findAll({where: {afk_level: 'warned2', status: true}}).then(users => {
                    ...
                    return Promise.all(users.map(user => user.update({afk_level: 'warned2'}))).then(() => //noinspection JSUnresolvedFunction
                    ...
                    return Promise.all(users.map(user => user.update({afk_level: 'warned'})));
                });
