...
class Database {
    constructor() {
        ...
        config.sequelize.options.logging = toLog => {
            if (config.options.sqlDebug) story.debug('sql', toLog);
        };
                ...
                return new Promise((resolve, reject) => {
                    ...
                    redis.hset('media:blacklist', `${format}:${cid}`, reason).then(() => that.models.Song.findOrCreate({
                        ...
                        defaults: {
                            ...
                            cid,
                            idBanned: true,
                            banReason: (reason === 1) ? null : reason
                        }
            ...
            remove: (format, cid) => {
                return new Promise((resolve, reject) => {
                    that.models.Song.find({where: {format, cid}}).then(song => {
                        if (song) {
                            return song.update({banReason: null, isBanned: false});
                        }
                        reject('not_found');
                    }).then(() => //noinspection JSUnresolvedFunction
        ...
        this.sequelize.validate().then(() => {
            ...
            redis.del('media:blacklist').then(() => //noinspection JSUnresolvedFunction
                that.models.Song.findAll({where: {isBanned: true}})).then(songs => Promise.all(songs.map(song => //noinspection JSUnresolvedFunction
                redis.hset(
                    'media:blacklist',
                    `${song.format}:${song.cid}`,
                    (song.banReason) ? song.banReason : 1
                )))).then(songs => {
                story.info(`Loaded blacklist with ${songs.length} entries.`);
                //noinspection JSUnresolvedFunction
                return that.models.Channel.findAll({where: {isBanned: true}});
