...
module.exports = {
    ...
    enabled: true,
    handler: (data) => {
        if (config.apiKeys.wordnik !== null && config.apiKeys.wordnik && config.apiKeys.wordnik !== '') {
            ...
            if (msg.length > 0) {
                ...
                request.get(uri, (error, response, body) => {
                    if (!error && response.statusCode === 200) {
                        ...
                        if (definition.length === 0) plugged.sendChat(utils.replace(langfile.define.noDefinitionFound, {
