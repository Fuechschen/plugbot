        ...
        redis.hget('user:roles',data.id).then(perm => {
            ...
            if (config.options.bouncerPlus ? (perm > 1) : (perm > 2)) {
                ...
                if (!isNaN(t)) {
                    ...
                    if (t > 1) setTimeout(() => {
                        plugged.sendChat(utils.replace(langfile.lottery.oneMinute, {username: data.username}));
                    }, (t - 1) * 60 * 1000);
                    setTimeout(() => {
                        ...
                        function choose() {
                            ...
                            redis.exists(`user:afk:${usr}`).then(ex => {
                                if (ex === 1) {
                                    ...
                                    if (wl.length === 1) plugged.sendChat(utils.replace(langfile.lottery.noWinner, {username: data.username}));
                                    else {
                                        wl = _.without(wl, usr);
                                        choose();
                                    }
