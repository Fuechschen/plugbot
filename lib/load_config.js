...
let conf;
try {
    conf = require('../config');  //eslint-disable-line global-require
} catch (e) {
...
else if (conf.options.commandPrefix === conf.customcommands.trigger) throw new Error('Commandprefix and customcommandtrigger must\'nt be th same');
else {
    module.exports = conf;

    let redis = new require('./db/redis_db');  //eslint-disable-line global-require
    let loaded = 0;
        ...
        redis.hexists('meta:config', 'lockskip:move_pos').then(ex => {
            if (ex) {
                ...
                redis.hget('meta:config', 'lockskip:move_pos').then(val => {
                    conf.lockskip.movePos = parseInt(val);
                    load('lockskippos', conf.lockskip.movePos);
                });
        ...
        redis.hexists('meta:config', 'options:bouncer_plus').then(ex => {
            if (ex) {
                ...
                redis.hget('meta:config', 'options:bouncer_plus').then(val => {
                    conf.options.bouncerPlus = val === '1';
                    load('bouncer+', conf.options.bouncerPlus);
                });
