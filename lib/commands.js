    ...
    handler: function (data) {
        redis.get('user:role:save:' + data.id).then(function (perm) {
            ...
            if (config.options.bouncer_plus ? (perm > 1) : (perm > 2)) {
                ...
                redis.set('meta:config:titleguard:enabled', (config.youtubeGuard.enabled ? 1 : 0));
            ...
            if (config.options.bouncer_plus ? (perm > 1) : (perm > 2)) {
                ...
                if (split.length === 2) {
                    ...
                    if (validator.isURL(split[1], {
                        ...
                        if (url.hostname === 'youtu.be') {
                        ...
                        } else if (url.hostname === 'www.youtube.com' && url.query.v !== undefined) {
                            console.log(url);
                            models.Song.find({where: {format: 1, cid: url.query.v}}).then(function (song) {
                            ...
                            request.get('http://api.soundcloud.com/resolve?url=' + split[1] + '&client_id=' + config.apiKeys.soundcloud, function (err, resp, body) {
                                if (!err && resp.statusCode === 200) {
                                    body = JSON.parse(body);
                                    models.Song.find({where: {format: 2, cid: body.id}}).then(function (song) {
                                        if (song !== undefined && song !== null) {
                                            utils.unblacklist(song.format, song.cid);
                                            plugged.sendChat(utils.replace(langfile.blacklist.unbl.default, {
                                                username: data.username,
                        ...
                        if (args.length === 2 && ['1', '2'].indexOf(args[1])) {
                            models.Song.find({
                                ...
                                if (song !== null && song !== undefined) {
                                    utils.unblacklist(args[0], args[1]);
                                    plugged.sendChat(utils.replace(langfile.blacklist.unbl.default, {
                                        username: data.username,
                                        song: utils.songtitle(song.author, song.title)
                                    }), 30);
