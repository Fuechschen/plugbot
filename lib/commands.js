    ...
    handler: function (data) {
        redis.get('user:role:save:' + data.id).then(function (perm) {
            ...
            if (config.options.bouncer_plus ? (perm > 1) : (perm > 2)) {
                ...
                if (user !== undefined) {
                    if (plugged.getWaitlist().indexOf(user.id) === -1 && (plugged.getCurrentDJ() !== undefined ? plugged.getCurrentDJ().id !== user.id : true )) {
    ...
    handler: function (data) {
        redis.get('user:role:save:' + data.id).then(function (perm) {
            ...
            if (config.options.bouncer_plus ? (perm > 1) : (perm > 2)) {
                ...
                if(split.length > 3){
                    if(split[1] === 'add'){
                        models.CustomCommand.findOrCreate({where: {trigger: split[2].toLowerCase()}, defaults: {trigger: split[2].toLowerCase(), message: _.rest(split, 3).join(' ').trim(), status: true}}).spread(function(cc){
                           cc.updateAttributes({message: _.rest(split, 3).join(' ').trim()});
                        });
                        plugged.sendChat(utils.replace(langfile.customcommands.created, {username: data.username, trigger: split[2].toLowerCase()}));
                    }
                } else if (split.length > 2){
                    if(split[1] === 'enable'){
                        models.CustomCommand.update({status: true}, {where: {trigger: split[2].toLowerCase()}});
                        plugged.sendChat(utils.replace(langfile.customcommands.cc_enabled, {username: data.username, trigger: split[2].toLowerCase()}));
                    } else if(split[1] === 'disable'){
                        models.CustomCommand.update({status: false}, {where: {trigger: split[2].toLowerCase()}});
                        plugged.sendChat(utils.replace(langfile.customcommands.cc_disabled, {username: data.username, trigger: split[2].toLowerCase()}));
                    } else if(split[1] === 'senderenable'){
                        models.CustomCommand.update({senderinfo: true}, {where: {trigger: split[2].toLowerCase()}});
                        plugged.sendChat(utils.replace(langfile.customcommands.senderinfo_enabled, {username: data.username, trigger: split[2].toLowerCase()}));
                    }else if(split[1] === 'senderdisable'){
                        models.CustomCommand.update({senderinfo: false}, {where: {trigger: split[2].toLowerCase()}});
                        plugged.sendChat(utils.replace(langfile.customcommands.senderinfo_disabled, {username: data.username, trigger: split[2].toLowerCase()}));
                    }
                }
            }
        });
...
commands.everyone = {
    handler: function (data) {
        redis.get('user:role:save:' + data.id).then(function (perm) {
            ...
            if (config.options.bouncer_plus ? (perm > 1) : (perm > 2)) {
                var split = data.message.trim().split(' ');
                plugged.sendChat(utils.replace(langfile.everyone.default, {username: data.username, msg: _.rest(split, 1).join(' ').trim()}));
            }
        });
        plugged.removeChatMessage(data.cid);
    }  
