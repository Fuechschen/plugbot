    ...
    handler: function (data) {
        redis.get('user:role:save:' + data.id).then(function (perm) {
            ...
            if (config.options.bouncer_plus ? (perm > 1) : (perm > 2)) {
                ...
                if(split.length > 3){
                    if(split[1] === 'add'){
                        models.CustomCommand.findOrCreate({where: {trigger: split[2].toLowerCase()}, defaults: {trigger: split[2].toLowerCase(), message: _.rest(split, 3).join(' ').trim(), active: true}}).spread(function(cc){
