        ...
        redis.get('user:role:save:' + msg.id).then(function (perm) {
            redis.exists('meta:state:skipable').then(function (ex) {
                ...
                if (perm > 1 && ex === 0) {
                    ...
                    redis.sismember('meta:data:rdjskip:votes', msg.id).then(function (is) {
                        if (is === 0) {
                                ...
                                if (active <= config.rdjskip.max_staff_active) {
                                    redis.scard('meta:data:rdjskip:votes').then(function (card) {
                                        if (card + 1 >= config.rdjskip.votes) {
                                            plugged.sendChat(utils.replace(langfile.rdjskip.skip, {username: msg.username}));
                                            story.info('skip', utils.userLogString(msg.username, msg.id) + ': ' + utils.mediatitlelog(media) + ' played by ' + utils.userLogString(plugged.getUserByID(booth.dj)));
                                        } else {
                                            redis.sadd('meta:data:rdjskip:votes', msg.id).then(function () {
                                                plugged.sendChat(utils.replace(langfile.rdjskip.default, {username: msg.username}));
                                            });
                                        }
                                    });
                                }
...
commands.cc = commands.customcommand = {
    handler: function (data) {
        redis.get('user:role:save:' + data.id).then(function (perm) {
            perm = parseInt(perm, 10);
            if (config.options.bouncer_plus ? (perm > 1) : (perm > 2)) {
                ...
                if (song !== null && song !== undefined) {
                    redis.exists('media:blacklist:' + song.format + ':' + song.cid).then(function (bl) {
                        redis.exists('media:history:' + song.format + ':' + song.cid).then(function (his) {
                           plugged.sendChat(utils.replace(langfile.songinfo.default, {username: data.username, title: utils.songtitle(song.author, song.title), bl: (bl === 1).toString(), his: (his === 1).toString(), sid: sid}));
                        });
                    });
                } else plugged.sendChat(utils.replace(langfile.error.argument, {
                    cmd: 'SongInfo',
