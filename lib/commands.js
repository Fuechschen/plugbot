        ...
        redis.exists('meta:roulette:active').then(function (ex) {
            if (ex === 1) {
                redis.sismember('meta:roulette:users', data.id).then(function (mem) {
                    ...
                    plugged.sendChat(utils.replace(langfile.roulette.already_joined, {username: data.username}));
                });
            }
            plugged.sendChat(utils.repalce(langfile.roulette.no_roulette, {username: data.username}));
        });
        plugged.removeChatMessage(data.cid);
    }
};
        ...
        redis.exists('meta:roulette:active').then(function (ex) {
            if (ex === 1) {
                redis.sismember('meta:roulette:users', data.id).then(function (mem) {
                    ...
                    plugged.sendChat(utils.replace(langfile.roulette.not_joined, {username: data.username}));
                });
            }
            plugged.sendChat(utils.repalce(langfile.roulette.no_roulette, {username: data.username}));
        });
                            ...
                            setTimeout(function () {
                                ...
                                redis.scard('meta:roulette:users').then(function (memcount) {
                                   if(memcount === 0){
                                       ...
                                       plugged.sendChat(utils.replace(langfile.roulette.started, {username: data.username, users: memcount}));
                                       if(memcount < m) m = memcount;
                                       redis.spop('meta:roulette:users', m).then(function (mem) {
                                           var users = [];
                                           if(m === 1) users.push(plugged.getUserByID(mem));
                                           else mem.forEach(function (member) {
                                               users.push(plugged.getUserByID(member));
                                           });
