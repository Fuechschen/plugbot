...
module.exports = {
    ...
    handler: function (data) {
                            ...
                            redis.incr('user:mute:' + data.id + ':violation').then(function () {
                                redis.get('user:mute:' + data.id + ':violation').then(function (val) {
                                    if (parseInt(val, 10) > config.chatfilter.spam.mute_violation) {
                                        plugged.muteUser(data.id, plugged.MUTEDURATION.LONG, plugged.BANREASON.SPAMMING, function (err) {
        ...
        webSockUpdate({m:data.message,u:{id:data.id,n:plugged.getUserByID(data.id)},cid:data.cid});
